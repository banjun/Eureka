...
public class PostalAddressCell<T: PostalAddressType>: Cell<T>, CellType, PostalAddressCellConformance, UITextFieldDelegate {
    ...
    public override func setup() {
        ...
        streetTextField.addTarget(self, action: #selector(PostalAddressCell.textFieldDidChange(textField:)), for: .editingChanged)
        stateTextField.addTarget(self, action: #selector(PostalAddressCell.textFieldDidChange(textField:)), for: .editingChanged)
        postalCodeTextField.addTarget(self, action: #selector(PostalAddressCell.textFieldDidChange(textField:)), for: .editingChanged)
        cityTextField.addTarget(self, action: #selector(PostalAddressCell.textFieldDidChange(textField:)), for: .editingChanged)
        countryTextField.addTarget(self, action: #selector(PostalAddressCell.textFieldDidChange(textField:)), for: .editingChanged)
    }
    ...
    override public var inputAccessoryView: UIView? {
        ...
        if let v = formViewController()?.inputAccessoryViewForRow(row: row) as? NavigationAccessoryView {
            if streetTextField.isFirstResponder() {
                v.nextButton.isEnabled = true
                v.nextButton.target = self
                v.nextButton.action = #selector(PostalAddressCell.internalNavigationAction(sender:))
            }
            else if stateTextField.isFirstResponder() {
                v.previousButton.target = self
                v.previousButton.action = #selector(PostalAddressCell.internalNavigationAction(sender:))
                v.nextButton.target = self
                v.nextButton.action = #selector(PostalAddressCell.internalNavigationAction(sender:))
    ...
    func internalNavigationAction(sender: UIBarButtonItem) {
        ...
        if streetTextField.isFirstResponder() {
            ...
            _ = sender == inputAccesoryView.previousButton ? streetTextField.becomeFirstResponder() : postalCodeTextField.becomeFirstResponder()
        }
        else if postalCodeTextField.isFirstResponder() {
            _ = sender == inputAccesoryView.previousButton ? stateTextField.becomeFirstResponder() : cityTextField.becomeFirstResponder()
        }
        else if cityTextField.isFirstResponder() {
            _ = sender == inputAccesoryView.previousButton ? postalCodeTextField.becomeFirstResponder() : countryTextField.becomeFirstResponder()
        }
        else if countryTextField.isFirstResponder() {
    ...
    public override func observeValue(forKeyPath keyPath: String?, of object: AnyObject?, change: [String : AnyObject]?, context: UnsafeMutablePointer<Void>?) {
        if let obj = object, let keyPathValue = keyPath, let changeType = change?[NSKeyValueChangeKindKey] where ((obj === titleLabel && keyPathValue == "text") || (obj === imageView && keyPathValue == "image")) && changeType.uintValue == NSKeyValueChange.setting.rawValue {
    ...
    public func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        formViewController()?.textInputDidBeginEditing(textInput: textField, cell: self)
    }
    ...
    public func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        return formViewController()?.textInputShouldReturn(textInput: textField, cell: self) ?? true
    }
    
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        return formViewController()?.textInputShouldEndEditing(textInput: textField, cell: self) ?? true
    }
    
    public func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        return formViewController()?.textInputShouldBeginEditing(textInput: textField, cell: self) ?? true
    }
    
    public func textFieldShouldClear(_ textField: UITextField) -> Bool {
        return formViewController()?.textInputShouldClear(textInput: textField, cell: self) ?? true
    }
